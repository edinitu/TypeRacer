...
public class hard_3 extends AppCompatActivity {
    ...
    public boolean verifyWord(CharSequence scris, CharSequence afisat) {
        StringBuilder sb = new StringBuilder(afisat.toString());
        if(scris.toString().equals(sb.reverse().toString())) return true;
        return false;
    }
    ...
    public void explainThenStart() {
        CountDownTimer c = new CountDownTimer(6000, 1000) {
            @Override
            public void onTick(long millisUntilFinished) {
                if(millisUntilFinished <= 3500) wordShowed.setText("Nivelul incepe in: " + millisUntilFinished/1000);
                else {
                    wordShowed.setTextSize(15);
                    if(Utils.LimbaRomana) {
                        wordShowed.setText("Scrie cuvintele de la dreapta la stanga");
                    } else if(Utils.EnglishLanguage) {
                        wordShowed.setText("Write the words in the opposite direction");
                    }
                }
            }
    ...
    public void nextLv() {
        if(noCorrect >= prag) {
            ...
            reference.child(Utils.player.getUsername()).child("level").setValue("champion");
            Utils.player.setLevel("champion");
